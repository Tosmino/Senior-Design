...
// This function sets very basic settings to get LED count values from the MAX86141.
...
uint8_t SparkFun_Bio_Sensor_Hub::configSensor(){
  ...
  statusChauf = max86141Control(ENABLE); //Enable Sensor.
  if( statusChauf != SFE_BIO_SUCCESS )
    return statusChauf;
...
// This function takes 9 bytes of LED values from the MAX86141 associated with
...
bioData SparkFun_Bio_Sensor_Hub::readSensor(){
  ...
  bioData libLedFifo;
  readFillArray(READ_DATA_OUTPUT, READ_DATA, MAX86141_LED_ARRAY, senArr);
...
bioData SparkFun_Bio_Sensor_Hub::readSensorBpm(){
  ...
  if (_userSelectedMode == MODE_ONE){
    ...
    readFillArray(READ_DATA_OUTPUT, READ_DATA, MAXFAST_ARRAY_SIZE + MAX86141_LED_ARRAY, bpmSenArr);
    ...
    // the sensor for LEDs that do not exists on the MAX86141. So we have to
    ...
    readFillArray(READ_DATA_OUTPUT, READ_DATA,\
        MAXFAST_ARRAY_SIZE + MAX86141_LED_ARRAY + MAXFAST_EXTENDED_DATA, bpmSenArrTwo);
...
// This function modifies the pulse width of the MAX86141 LEDs. All of the LEDs
...
uint8_t SparkFun_Bio_Sensor_Hub::setPulseWidth(uint16_t width){
  ...
  regVal = readRegisterMAX86141(CONFIGURATION_REGISTER);
  regVal &= PULSE_MASK; // Mask bits to change.
  regVal |= bits; // Add bits
  writeRegisterMAX86141(CONFIGURATION_REGISTER, regVal); // Write Register
...
// MAX86141 Sensor. It returns one of the four settings in microseconds.
...
// This function changes the sample rate of the MAX86141 sensor. The sample
// rate is affected by the set pulse width of the MAX86141 LEDs.
// Default: 69us - 15 resolution - 50 samples per second.
...
// MAX86141 Sensor. It returns one of the 8 possible sample rates.
...
// MAX86141 Register: CONFIGURATION_REGISTER (0x0A), bits [6:5]
// This functions sets the dynamic range of the MAX86141's ADC. The function
...
uint8_t SparkFun_Bio_Sensor_Hub::setAdcRange(uint16_t adcVal){
  ...
  writeRegisterMAX86141(CONFIGURATION_REGISTER, regVal);

  return SFE_BIO_SUCCESS;
}

// MAX86141 Register: CONFIGURATION_REGISTER (0x0A), bits [6:5]
// This function returns the set ADC range of the MAX86141 sensor.
...
// Family Byte: ENABLE_SENSOR (0x44), Index Byte: ENABLE_MAX86141 (0x00), Write
// Byte: senSwitch  (parameter - 0x00 or 0x01).
// This function enables the MAX86141.
uint8_t SparkFun_Bio_Sensor_Hub::max86141Control(uint8_t senSwitch) {
...
// Family Byte: READ_SENSOR_MODE (0x45), Index Byte: READ_ENABLE_MAX86141 (0x03)
// This function checks if the MAX86141 is enabled or not.
uint8_t SparkFun_Bio_Sensor_Hub::readMAX86141State(){

  uint8_t state = readByte(READ_SENSOR_MODE, READ_ENABLE_MAX86141);
  return state;
}
...
// Family Byte: WRITE_REGISTER (0x40), Index Byte: WRITE_MAX86141 (0x03), Write Bytes:
// Register Address and Register Value
// This function writes the given register value at the given register address
// for the MAX86141 sensor and returns a boolean indicating a successful or
// non-successful write.
void SparkFun_Bio_Sensor_Hub::writeRegisterMAX86141(uint8_t regAddr, uint8_t regVal) {

  writeByte(WRITE_REGISTER, WRITE_MAX86141, regAddr, regVal);
...
// Family Byte: READ_REGISTER (0x41), Index Byte: READ_MAX86141 (0x03), Write Byte:
// Register Address
// This function reads the given register address for the MAX86141 Sensor and
// returns the values at that register.
uint8_t SparkFun_Bio_Sensor_Hub::readRegisterMAX86141(uint8_t regAddr) {

  uint8_t regCont = readByte(READ_REGISTER, READ_MAX86141, regAddr);
  return regCont;
...
// Family Byte: READ_ATTRIBUTES_AFE (0x42), Index Byte: RETRIEVE_AFE_MAX86141 (0x03)
// This function retrieves the attributes of the AFE (Analog Front End) of the
// MAX86141 sensor. It returns the number of bytes in a word for the sensor
// and the number of registers available.
sensorAttr SparkFun_Bio_Sensor_Hub::getAfeAttributesMAX86141() {

  sensorAttr maxAttr;
  uint8_t tempArray[2] {};

  readFillArray(READ_ATTRIBUTES_AFE, RETRIEVE_AFE_MAX86141, 2, tempArray);
...
// Family Byte: DUMP_REGISTERS (0x43), Index Byte: DUMP_REGISTER_MAX86141 (0x03)
// This function returns all registers and register values sequentially of the
// MAX86141 sensor: register zero and register value zero to register n and
// register value n. There are 36 registers in this case.
uint8_t SparkFun_Bio_Sensor_Hub::dumpRegisterMAX86141(uint8_t regArray[]) {

  uint8_t numOfBytes = 36;
  uint8_t status = readFillArray(DUMP_REGISTERS, DUMP_REGISTER_MAX86141, numOfBytes, regArray);
  return status;
